...
// 짧은 시간에 다수 위치 인증 = 가짜 좌표 대량 인증 의심 → 운영자 보고(차단 아님)
export const RAPID_LOCATION_WINDOW_MS = 60 * 60 * 1000; // 1시간
export const RAPID_LOCATION_COUNT = 3; // 1시간에 이 수 이상 인증되면 의심 플래그
...
export async function verifyLocation(
  ...
  // 인증은 통과시키되, 짧은 시간 다수 인증이면 운영자 의심 플래그를 남긴다(차단 아님).
  await flagRapidLocationIfNeeded(userId).catch((e) => console.error("[verify] rapid-flag 실패", e));
  return { verified: true, reason: "OK", distanceMeters: dist, awardedXp, ...base };
}

/**
 * 짧은 시간(1시간)에 위치 인증이 RAPID_LOCATION_COUNT 이상이면 어뷰징 의심 플래그 생성.
 * 가짜 좌표로 대량 인증하는 판매자격 어뷰징을 운영자가 잡도록 보고만 한다(정상 이용 차단 X).
 * 같은 사용자에 대해 미검토(open) 플래그가 이미 있으면 카운트만 갱신(중복 방지).
 */
async function flagRapidLocationIfNeeded(userId: string): Promise<void> {
  const since = new Date(Date.now() - RAPID_LOCATION_WINDOW_MS);
  const recent = await prisma.proofAttempt.count({
    where: { userId, kind: "location", ok: true, createdAt: { gte: since } },
  });
  if (recent < RAPID_LOCATION_COUNT) return;

  const detail = `1시간 내 위치 인증 ${recent}곳 (기준 ${RAPID_LOCATION_COUNT}곳)`;
  const existing = await prisma.abuseFlag.findFirst({
    where: { userId, kind: "rapid_location", status: "open" },
    select: { id: true },
  });
  if (existing) {
    await prisma.abuseFlag.update({ where: { id: existing.id }, data: { count: recent, detail } });
  } else {
    await prisma.abuseFlag.create({
      data: { userId, kind: "rapid_location", count: recent, detail },
    });
  }
}
