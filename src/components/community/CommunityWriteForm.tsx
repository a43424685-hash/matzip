...
export default function CommunityWriteForm({ initialCategory }: { initialCategory?: string }) {
  ...
  const [video, setVideo] = useState<{ url: string; thumb: string | null } | null>(null);
  ...
  async function onPickVideo(file: File | null) {
    ...
    try {
      ...
      setVideo({ url: up.url, thumb: up.thumbnailUrl });
    } catch (e) {
      setErr(e instanceof Error ? e.message : "영상 업로드에 실패했어요.");
    }
    setUploading(false);
  }
  ...
  async function submit() {
    ...
    const r = await fetch("/api/community", {
      ...
      body: JSON.stringify({
        ...
        videoThumbUrl: video?.thumb ?? null,
      }),
    });
    ...
    <div className="space-y-4">
      ...
      {(images.length > 0 || video) && (
        <div className="flex flex-wrap gap-2">
          ...
          {video && (
            <div className="relative h-20 w-20 overflow-hidden rounded-xl bg-stone-800">
              {video.thumb && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={video.thumb} alt="" className="h-full w-full object-cover opacity-80" />
              )}
              <Video size={16} className="absolute inset-0 m-auto text-white" />
              <button
                type="button"
      ...
      <div className="flex gap-2">
        ...
        {uploading && <Loader2 size={18} className="mt-2 animate-spin text-forest" />}
      </div>
      <input ref={imgInput} type="file" accept="image/*" multiple hidden onChange={(e) => onPickImages(e.target.files)} />
      <input ref={vidInput} type="file" accept="video/*" hidden onChange={(e) => onPickVideo(e.target.files?.[0] ?? null)} />
