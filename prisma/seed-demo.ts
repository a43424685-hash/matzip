/**
 * 데모 데이터 시드 — 프리뷰/테스트에서 랭킹·프로필·유료지도를 채우기 위한 가짜 데이터.
 * 결정적 ID(demo-*) 사용 → 재실행해도 중복 안 생김(upsert).
 * 실행: DATABASE_URL 설정 후  npx tsx prisma/seed-demo.ts
 * 정리(삭제): npx tsx prisma/seed-demo.ts --clean
 */
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

// 레벨 내림차순 — 랭킹이 보기 좋게 퍼지도록
const FOODIES = [
...
const SPOTS = [
  ...
  { name: "종로 국밥노포", short: "30년 내공 국물", tags: ["노포", "국밥/탕"], price: "under_10k" },
  ...
  { name: "여의도 회센터", short: "퇴근 후 회 한 점", tags: ["회식", "회/해산물"], price: "20k_40k" },
...
async function main() {
  ...
  if (!seoul) throw new Error("서울 지역이 없습니다. 먼저 `npm run db:seed` 실행하세요.");
    ...
    for (let j = 0; j < 3; j++) {
      ...
      await prisma.restaurant.upsert({
        ...
        create: {
          ...
          address: "서울특별시",
          latitude: 37.55 + Math.random() * 0.05,
          longitude: 126.97 + Math.random() * 0.05,
      ...
      await prisma.restaurantPost.upsert({
        ...
        create: {
          ...
          saveCount: Math.floor(Math.random() * 90),
  ...
  for (let u = 0; u < 3; u++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      await prisma.collectionItem.upsert({
        ...
        update: {},
        create: { id: `demo-ci-${u + 1}-${j + 1}`, collectionId: cid, restaurantId: rid, postId: pid, isPreview: j < 2, sortOrder: j },
      });
    }
  }

  const users = await prisma.user.count({ where: { id: { startsWith: "demo-u" } } });
  const posts = await prisma.restaurantPost.count({ where: { id: { startsWith: "demo-p" } } });
  const maps = await prisma.collection.count({ where: { id: { startsWith: "demo-c" } } });
  console.log(`데모 시드 완료. 유저 ${users}명, 글 ${posts}개, 유료지도 ${maps}개`);
}
